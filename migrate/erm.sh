...
# due to macOS. unix can just use `head -n (negative value)`
inverse_tail () {
  sed -n -e ':a' -e "1,$1!{P;N;D;};N;ba"
}

psql "postgres://app_user:password@localhost/at_uconn" -c "
...
WHERE c.table_schema = 'test' AND t.table_type = 'BASE TABLE'" | sed 's/, /<BR\/>/g' | inverse_tail 2 | tail -n+3 | awk -F"|" '
function ltrim(s) {
    sub(/^[ \t\r\n]+/, "", s);
    return s
}
